...
if [ ! -f "$build_dir/cmake-${cmake_version}.tar.gz" ]; then
    wget $cmake_url -O "$build_dir/cmake-${cmake_version}.tar.gz"
fi
if [ ! -d "$build_dir/cmake-${cmake_version}" ]; then
    (cd $build_dir && tar zxvf cmake-${cmake_version}.tar.gz)
fi
if [ ! -f "$cmake_binary" ]; then
    (cd $build_dir/cmake-${cmake_version} && ./configure --prefix=$install_dir/cmake && make && make install)
fi
if [ $? -ne 0 ]; then
...
#   - libegl1-mesa-dev
###############################################################################
if [ ! -f "$build_dir/SDL-release-${sdl2_version}.tar.gz" ]; then
    wget $sdl2_url -O "$build_dir/SDL-release-${sdl2_version}.tar.gz"
fi
if [ ! -d "$build_dir/SDL-release-${sdl2_version}" ]; then
    (cd $build_dir && tar zxvf SDL-release-${sdl2_version}.tar.gz)
fi
if [ ! -d "$build_dir/SDL-release-${sdl2_version}/build" ]; then
    mkdir $build_dir/SDL-release-${sdl2_version}/build
fi
(cd $build_dir/SDL-release-${sdl2_version}/build && \
...
###############################################################################
if [ ! -d "$build_dir/SDL_mixer" ]; then
    (cd $build_dir && git clone --recurse-submodules --depth 1 --branch $sdl_mixer_version $sdl_mixer_url)
fi
if [ ! -d "$build_dir/SDL_mixer/build" ]; then
    mkdir $build_dir/SDL_mixer/build
fi
(cd $build_dir/SDL_mixer/build && \
    $cmake_binary \
...
# Setup
$build_dir/linuxdeploy.AppImage -e $build_dir/SoundSphere/soundsphere -d $project_dir/tools/io.github.qgymib.soundsphere.desktop -i $project_dir/docs/icon_256.png --appdir=$app_dir
...
(cd $build_dir && $build_dir/linuxdeploy.AppImage --appdir=$app_dir --output appimage)
if [ $? -ne 0 ]; then
